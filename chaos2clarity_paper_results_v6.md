...
|---|---|---|---|---|
| T=50 (batch 1) | **66.0%** (66.5 ± 2.0) | 60.0% (60.0 ± 0.0) | 24.0% (23.0 ± 2.0) | 32.0% (32.0 ± 0.0) |
| T=100 (batch 2) | **80.0%** (79.5 ± 1.0) | 60.0% (60.0 ± 0.0) | 14.0% (15.0 ± 2.0) | 32.0% (32.0 ± 0.0) |
| T=150 (batch 3) | **86.0%** (87.0 ± 2.0) | 60.0% (60.0 ± 0.0) | 12.0% (11.0 ± 2.0) | 32.0% (32.0 ± 0.0) |
| T=200 (batch 4) | **88.0%** (88.5 ± 1.0) | **60.0%** (60.0 ± 0.0) | **10.0%** (8.5 ± 3.0) | **32.0%** (32.0 ± 0.0) |
...
**Result analysis: Prediction confirmed.** EA(C2C-Full, T=200) = 88.5% ± 1.0 ≥ EA(C2C-Full, T=50) + 5pp = 71.5%. Actual improvement: **+22pp** (≥4× the predicted minimum). ABL-NoFeedback is statistically flat at 60.0% ± 0.0 across all four checkpoints. The near-zero variance across 4 runs confirms the stability of the self-improvement effect.

**Feedback loop statistics (T=200):** 415 total feedback signals processed; 893 κ updates applied; 12 few-shot E1 corrections and 12 E2 corrections injected; 24 schema enrichment proposals generated. E1 hallucination rate drops from 24% to 10% (−14pp) while E5 cross-source failure drops from 8% to 2%.
...
| Checkpoint | C2C-Full First-Pass EA | ABL-NoVector First-Pass EA |
|---|---|---|
| T=50 | 66.0% | 56.0% |
| T=100 | **78.0%** | 56.0% |
| T=150 | **86.0%** | 58.0% |
...
**Result: Prediction confirmed.** By T=100, first-pass EA gap = 78% − 56% = **22pp**, far exceeding the predicted ≥8pp. By T=200, C2C-Full has a +24pp first-pass advantage. ABL-NoVector shows only marginal improvement (+6pp over 200 queries) from feedback alone, confirming that the vector store is the primary driver of first-pass accuracy gains.
...
| C2C-Full (T=200) | **10.0** | **58.0** | **0.0** | **0.0** | **2.0** | **88.0** |
...
**Finding 2: Vector grounding suppresses hallucination over time.** E1 (schema hallucination) drops from 24% to 10% as $\mathcal{V}$ accumulates verified query patterns. The +24pp first-pass advantage over ABL-NoVector (86% vs 62% by T=200) confirms that verified execution pattern retrieval is a viable, deployment-safe alternative to model fine-tuning.
...
All experiments run locally on an Apple M2 Mac using Ollama with Qwen 2.5 Coder 3B (temperature=0, deterministic). No cloud APIs, no external costs. Results were replicated across two independent runs with identical outcomes. Prototype implementation, prompt templates, the 50-question BI suite with gold SQL, evaluation harness, and all raw result data are released at:
