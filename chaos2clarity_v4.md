...
We formalize the problem, deploy a prototype over a realistic three-source retail environment (PostgreSQL, Salesforce CRM export, logistics CSV — 47 columns, zero pre-existing documentation), derive a five-class error taxonomy from prototype operation, and specify an eight-experiment evaluation protocol with explicit falsifiable predictions and empty result tables awaiting experimental runs. Based on the structural analysis of failure modes in uncurated BI settings, we hypothesize that C2C will improve execution accuracy by ≥ 25 percentage points over a direct LLM-to-SQL baseline and will enable cross-source queries that the baseline cannot execute.
...
This hypothesis is operationalized in Section 8 via eight experiments, each targeting a specific architectural component with a falsifiable prediction and explicit failure condition.
...
| Semantic graph | Neo4j 5.x | Mechanism I |
...
We define eight experiments: six core experiments each mapping directly to an architectural claim, and two secondary experiments for system characterization. **All result tables below contain empty cells to be filled upon experimental runs** (see companion experiments specification document for full setup).
...
**Prediction:** EA(C2C-Full) ≥ EA(𝔅₁) + 25pp overall. EA(C2C-Full) on L3 > 0% while EA(𝔅₁) on L3 = 0% (cross-source queries are categorically unsolvable without a semantic layer).  
...
**Predictions:** E1 rate significantly lower in C2C-Full than 𝔅₂ (synonym resolution); E3 rate significantly lower in C2C-Full than 𝔅₃ (cross-source join grounding); E5 rate = 0 in C2C-Full on L3.
...
|---|---|---|---|---|---|
| 𝔅₂: Schema-aware LLM (no decomposition, no 𝒮) | — | — | — | — | — |
...
**Predictions:** EA(C2C-Full) > EA(ABL-Mono) on L2–L3 isolating decomposition benefit; removing Validator increases policy violations to > 0; EA(ABL-NoRetry) < EA(C2C-Full) demonstrating retry contribution.
...
**Prediction:** C2C-Full's degradation (Structured → Heterogeneous) < 50% of 𝔅₁'s degradation.
...
**Expected output:** Graph with X-axis = query batch T, Y-axis = EA (left) and E1 rate (right). C2C-Full should show a positive trend in EA and declining E1 rate; ABL-NoFeedback should be flat.  
**Prediction:** EA(C2C-Full, T=200) ≥ EA(C2C-Full, T=50) + 5pp; EA(ABL-NoFeedback) is statistically flat across all checkpoints (Mann-Whitney U, $\alpha = 0.05$).  
...
**Prediction:** By T=100, first-pass EA(C2C-Full) > first-pass EA(ABL-NoVector) by ≥ 8pp on L1+L2; E1 rate in C2C-Full declines faster than in ABL-NoVector across the four checkpoints.
...
**Expected output:** Stacked bar chart. Validates that different architectural decisions suppress different error classes as the taxonomy predicts.
...
**Expected insight:** "C2C trades X ms P50 latency for Y pp accuracy improvement. On warm query distributions (cache hit rate ≥ Z%), P50 falls below the baseline."
...
Inject four controlled schema perturbations per Dr. Spider taxonomy [33]:
...
LLM-inferred semantic mappings may exhibit hallucination or inconsistency on ambiguous schemas [21, 19]. Multi-source query planning incurs latency proportional to cross-system join complexity. The feedback loop requires reaching batch threshold $\theta_\text{batch}$ before updating, creating a cold-start period for rare error types. Vector store grounding provides no benefit before warm-up queries have been processed. The prototype and evaluation are confined to a single domain (retail); generalization to other domains is deferred to the extended version.

### Future Work

1. Extending 𝓛₂ with domain ontologies (FIBO for finance, HL7/FHIR for healthcare) to reduce bootstrap time.
2. Integrating differential privacy [34] into $f_\text{vrf}$ for PII-sensitive deployments where masking alone is insufficient.
...
The paper presents a deployed three-source retail prototype, a five-class error taxonomy, and an eight-experiment evaluation protocol with explicit falsifiable predictions. C2C addresses the gap identified by recent data agent surveys [3, 5] between current AI-over-data systems and the heterogeneous realities of enterprise data environments.
...
[11] Abai, N. et al. Enterprise Data Modeling Effort Survey. *(Replace with specific citation documenting manual modeling effort costs; Singh et al. [21] partially covers this.)*  
...
- [ ] Register at orcid.org; replace `0000-0000-0000-0000`
- [ ] Replace `ravi@example.com` with real email
- [ ] Create `github.com/bankupalliravi/chaos2clarity`; push code, prompts, TikZ sources, gold question suite
- [ ] Run all eight experiments; fill every `—` cell before arXiv upload
- [ ] Fill Appendix F (α sensitivity) from validation run
- [ ] Replace reference [11] with a specific citation for semantic layer manual effort cost
- [ ] Verify all 34 references are unique (no duplicates)
- [ ] Update LaTeX source to match v4: Definitions 5–7, ablation variant naming (ABL-*), Prop 2 second sentence, "to our knowledge" qualifiers, Appendix F
- [ ] Test LaTeX compilation: `pdflatex → bibtex → pdflatex → pdflatex`
- [ ] Verify `acmart` compiles on arXiv; fallback to `\documentclass[preprint]{acmart}`
